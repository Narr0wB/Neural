...
int Matrix::resize(size_t new_rows, size_t new_cols)
{
    ...
    if (new_rows < _rows) {
        for (size_t i = new_rows; i < _rows; ++i)
            delete data[i];
    }
...
Matrix Matrix::flatten(int row_or_cols, int options) const
{
    ...
    if (row_or_cols == COLUMN) { 
        ...
        Matrix flat(1, this->_rows * this->_cols);

        if (options == COLUMN_FLATTEN)
        { 
            for (size_t j = 0; j < this->_cols; j++)
                for (size_t i = 0; i < this->_rows; i++)
                    flat.data[0][j * this->_rows + i] = this->data[i][j];
        }
        else 
        {
            for (size_t i = 0; i < this->_rows; i++)
                for (size_t j = 0; j < this->_cols; j++)
                    flat.data[0][i * this->_cols + j] = this->data[i][j];
        }
